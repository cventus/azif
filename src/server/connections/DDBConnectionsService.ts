  ...
  ({ DocumentClient: client, TableConfig, Logger }): ConnectionsService => {
    ...
    return Logger.traceMethods(logger, {
      async removeConnection(socketId, gameId) {
        const { Attributes: item } = await client.delete({
      ...
      async getConnection(socketId) {
        ...
        if (!isSocketItem(item)) {
          const error: Error & Partial<Record<'item', unknown>> = new Error('Not a SocketItem')
          error.item = item
          throw error
        }
        return {
      ...
      async getGameConnections(gameId) {
        ...
        if (mismatch) {
          const error: Error & Partial<Record<'item', unknown>> = new Error('Not a SocketListItem')
          error.item = mismatch
          throw error
        }
      ...
      async setGame(socketId, gameId) {
        // Update socket entry
        const { Attributes: oldItem } = await client.update({
          Key: { id: `socket:${socketId}` },
          TableName: ItemTableName,
        ...
        await client.put({
          Item: { gameId, socketId, userId: oldItem.userId, ttl: oldItem.ttl },
          TableName: ListTableName,
        }).promise()
