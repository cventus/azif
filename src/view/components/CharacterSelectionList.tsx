...
const compareCharacters = (a: CharacterCard, b: CharacterCard): number =>
...
const CharacterSelectionList: React.FC<{
    ...
    () =>
      game.contentSetIds
        .reduce<CharacterCard[]>((acc, id) => {
          if (content[id]) {
            const foo = Object.values(content[id].cards).filter(
              (card: Card): card is CharacterCard => card.type === 'character',
            )
            return [...acc, ...foo]
          } else {
            return acc
  ...
  const characterPlayedBy = Object.entries(game.players).reduce<
  ...
  const onFocusCharacter = useCallback(
  ...
  const current =
    characters.find((c) => c.id === focusedCharId) || characters[0]

  return (
    <CharacterContainer>
      <CharacterList>
        <form onSubmit={selectCharacter}>{options}</form>
      </CharacterList>
      <CharacterInfo>
        <h1>{current.name}</h1>
        <p>{current.title}</p>
        <h3>Story</h3>
        <section>
          {current.story.map((paragraph, i) => (
            <p key={i}>{paragraph}</p>
          ))}
        </section>
        <h3>Special Ability</h3>
        <section>
          {current.specialAbility.description?.map((paragraph, i) => (
            <p key={i}>{paragraph}</p>
          ))}
          {current.specialAbility.action && (
            <p>
              <b>Action:</b>
              {current.specialAbility.action.map((text, i) => (
                <>
                  {' '}
